...
def vector_to_components(
    ...
    for joint in range(1, 7):
        j = joint - 1
        comp["delta_alpha"][j] = by_name[f"delta_alpha_{joint}"]
        comp["delta_a"][j] = by_name[f"delta_a_{joint}"]
        comp["delta_d"][j] = by_name[f"delta_d_{joint}"]
        comp["delta_theta"][j] = by_name[f"delta_theta_{joint}"]
        comp["rrd"][j] = by_name[f"delta_rrd_{joint}"]
        comp["backlash"][j] = by_name[f"delta_backlash_{joint}"]
        comp["flex"][j] = by_name[f"delta_flex_{joint}"]

    for index, axis in enumerate(("x", "y", "z")):
        comp["base_xyz"][index] = by_name[f"delta_Bt{axis}"]
        comp["base_rpy"][index] = by_name[f"delta_Bu{axis}"]
        comp["tool_xyz"][index] = by_name[f"delta_Tt{axis}"]
        comp["tool_rpy"][index] = by_name[f"delta_Tu{axis}"]
    return comp
