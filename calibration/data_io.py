...
def load_dataset(path: str | Path) -> dict[str, Any]:
    ...
    keys include ``payloads``, ``directions``, and simulation truth metadata.
    """
    ...
    if isinstance(raw, dict):
        ...
        directions = raw.get("directions", raw.get("approach_directions", raw.get("h", None)))
        ...
        directions = [item.get("direction", np.sign(item["joints"])) for item in raw]
        extra = {}
    else:
